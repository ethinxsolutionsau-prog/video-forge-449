...
export default function ScriptPanel({ projectId, script, canEdit, onChange }) {
  ...
  const generate = async () => {
    setGenerating(true);
    toast.loading("Generating script…", { id: "gen-script" });
    try {
      const { data } = await api.post(`/projects/${projectId}/generate-script`);
      onChange(data);
      const src = data?.script?.generation_source || "fallback";
      toast.success(`Script generated · ${src}`, { id: "gen-script" });
    } catch (err) {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4 font-mono text-[11px] text-zinc-500">
          ...
          {script.generation_source && (
            <>
              <span>·</span>
              <span
                data-testid="script-generation-source"
                className={`uppercase tracking-widest text-[10px] px-1.5 py-0.5 rounded-sm border ${
                  script.generation_source === "fallback"
                    ? "border-zinc-700 text-zinc-400"
                    : "border-[#00E5FF]/40 text-[#00E5FF]"
                }`}
              >
                Generated by: {script.generation_source}
              </span>
            </>
          )}
        </div>
